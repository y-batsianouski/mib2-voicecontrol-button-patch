...
/**
 * SHADOW of the OEM class of the same name, loaded via -Xbootclasspath/p:.
 *
 * A shadow REPLACES the OEM class outright; there is no superclass to delegate to.
 * Therefore every member below reproduces the OEM implementation EXACTLY, including
 * access modifiers (the constructor is package-private, deactivate() is private) and
 * the ABORT_KEYS contents. The ONLY additions are
 * the activate() suppression gate and the install() call. Any other divergence is a bug.
 */
...
implements DialogSessionListener {
    ...
    public void activate() {
        if (vcbpatch.Remap.shouldSuppressAndPark(this)) {
            return;
        }
